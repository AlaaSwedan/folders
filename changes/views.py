...
# add new views by deltasoft
def subjects(request):
    subjects=CourseOverview.objects.all().values_list('course_subject', flat=True).distinct()
    return render_to_response("courseware/subjects.html", {'subjects': subjects})

def info_technology(request):
    courses_list = {}
    courses_list = get_info_technology(request.user)
    return render_to_response("courseware/subject.html",{'courses': courses_list})

def medical(request):
    courses_list = {}
    courses_list = get_medical(request.user)
    return render_to_response("courseware/subject.html",{'courses': courses_list})

def programming(request):
    courses_list = {}
    courses_list = get_programming(request.user)
    return render_to_response("courseware/subject.html",{'courses': courses_list})
# *************
